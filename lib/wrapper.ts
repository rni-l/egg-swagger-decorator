...
const validator = (parameters: Parameters) => async(ctx, next) => {
  ...
  handleSwagger = (router: Router, options: WrapperOptions) => {
    const {
...
const handleMap = (app: Application, ControllerClass: typeof Controller) => {
    const anonymousContext = app.createAnonymousContext(),
      router = app.router,
      c: Controller = new ControllerClass(Object.assign(anonymousContext)),
      methods: string[] = Object.getOwnPropertyNames(Object.getPrototypeOf(c));
  ...
  wrapper = (app: Application, options?: WrapperOptions) => {
  ...
  makeSwaggerRouter = (app: Application) => handleMapDir(app);
